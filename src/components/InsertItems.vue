      ...
      <div v-for="row in mappedData" v-bind:key="row.fieldName" class="row gutter-x-md">
        ...
        <div class="col-md-6">
          ...
          <q-field v-if="row.type === 7 ">
            <q-select v-model="row.data" v-if="selectCompagnie.length > 1 && row.fieldName === 'printCompany'"
                      :options="selectCompagnie"/>
            <q-select v-model="row.data" v-if="selectAttTypes.length > 1 && row.fieldName === 'AttachmentTypes'"
                      :options="selectAttTypes"/>
            <q-select v-model="row.data" v-if="selectPackageSpline.length > 1 && row.fieldName === 'packageSpline'"
                      :options="selectPackageSpline"/>
            <q-select v-model="row.data"
                      v-if="selectPackageSplineOnu.length > 1 && row.fieldName === 'packageSplineONU'"
                      :options="selectPackageSplineOnu"/>
            <q-select v-model="row.data"
                      v-if="selectPackageSplineOnu2.length > 1 && row.fieldName === 'packageSplineONU2'"
                      :options="selectPackageSplineOnu2"/>
            <q-select v-model="row.data" v-if="selectPackageType.length > 1 && row.fieldName === 'packageType'"
                      :options="selectPackageType"/>
            <q-select v-model="row.data" v-if="selectSimStatus.length > 1 && row.fieldName === 'simStatus'"
                      :options="selectSimStatus"/>
            <q-select v-model="row.data" v-if="selectSemStatus.length > 1 && row.fieldName === 'semStatus'"
                      :options="selectSemStatus"/>
            <q-select v-model="row.data" v-if="selectQuoteStatus.length > 1 && row.fieldName === 'quoteStatus'"
                      :options="selectQuoteStatus"/>
            <q-select v-model="row.data" v-if="selectLabStatus.length > 1 && row.fieldName === 'labStatus'"
                      :options="selectLabStatus"/>
            <q-select v-model="row.data" v-if="selectCustomer.length > 1 && row.fieldName === 'customer'"
                      :options="selectCustomer"/>
            <q-select v-model="row.data" v-if="selectPallStack.length > 1 && row.fieldName === 'palletStacking'"
                      :options="selectPallStack"/>
            <q-select v-model="row.data" v-if="selectPallType.length > 1 && row.fieldName === 'palletType'"
                      :options="selectPallType"/>
            <q-select v-model="row.data" v-if="selectMecPacktype.length > 1 && row.fieldName === 'mechanisedPackaging'"
                      :options="selectMecPacktype"/>
            <q-select v-model="row.data" v-if="selectAnnualMarket.length > 1 && row.fieldName === 'annualMarket'"
                      :options="selectAnnualMarket"/>
            <q-select v-model="row.data" v-if="selectQuantityChoice.length > 1 && row.fieldName === 'quantityChoice'"
                      :options="selectQuantityChoice"/>
            <q-select v-model="row.data" v-if="selectStampLne.length > 1 && row.fieldName === 'stampLne'"
                      :options="selectStampLne"/>
            <q-select v-model="row.data" v-if="selectPantoneType.length > 1 && row.fieldName === 'pantoneType'"
                      :options="selectPantoneType"/>
            <q-select v-model="row.data" v-if="selectPantoneType2.length > 1 && row.fieldName === 'pantoneType2'"
                      :options="selectPantoneType2"/>
            <q-select v-model="row.data" v-if="selectSupportType.length > 1 && row.fieldName === 'supportType'"
                      :options="selectSupportType"/>
            <q-select v-model="row.data" v-if="selectColorType.length > 1 && row.fieldName === 'tenColorType'"
                      :options="selectColorType"/>
            <q-select v-model="row.data" v-if="selectRackType.length > 1 && row.fieldName === 'rackType'"
                      :options="selectRackType"/>
            <q-select v-model="row.data" v-if="selectDelCity.length > 1 && row.fieldName === 'deliveryCity'"
                      :options="selectDelCity"/>
            <q-select v-model="row.data" v-if="selectDelCity2.length > 1 && row.fieldName === 'deliveryCity2'"
                      :options="selectDelCity2"/>
            <q-select v-model="row.data" v-if="selectDelType.length > 1 && row.fieldName === 'deliveryType'"
                      :options="selectDelType"/>
            <q-select v-model="row.data" v-if="selectPicType.length > 1 && row.fieldName === 'huePictureType'"
                      :options="selectPicType"/>
            <q-select v-model="row.data" v-if="selectTransportType.length > 1 && row.fieldName === 'transportType'"
                      :options="selectTransportType"/>
            <q-select v-model="row.data" v-if="selectProductNature.length > 1 && row.fieldName === 'productNature'"
                      :options="selectProductNature"/>
            <q-select v-model="row.data" v-if="selectProductNature2.length > 1 && row.fieldName === 'productNature2'"
                      :options="selectProductNature2"/>
            <q-select v-model="row.data" v-if="selectStack.length > 1 && row.fieldName === 'stacking'"
                      :options="selectStack"/>
            <q-select v-model="row.data" v-if="selectDelPlace.length > 1 && row.fieldName === 'deliveryPlace'"
                      :options="selectDelPlace"/>
            <q-select v-model="row.data" v-if="selectTruckType.length > 1 && row.fieldName === 'truckType'"
                      :options="selectTruckType"/>
          </q-field>
  ...
  methods: {
    ...
    open () {
      ...
      for (let i = 0; i < this.$parent.Fields.length; i++) {
        vm.mappedData.push({
          nameForPost: this.$parent.Fields[i].name,
          fieldName: this.$parent.Fields[i].label,
          data: '',
          type: this.$parent.Fields[i].type
        })
      }
    ...
    getUsersAndGroups () {
      this.$http.api.lists.getUsers().then(response => {
        ...
        for (let i = 0; i < this.Users.length; i++) {
          this.listTest.push({
            name: this.Users[i].Title,
            mail: this.Users[i].Email,
            icon: `person`,
            id: this.Users[i].Id
          })
        }
